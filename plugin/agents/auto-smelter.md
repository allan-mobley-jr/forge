...
1. Document the team-level shared resource provisioning procedure (Neon, Blob, etc.) and the per-hub Vercel project setup procedure in INGOT.md's Deployment & Environments section (see step 6).
2. Embed all Vercel infrastructure acceptance criteria in the **restructure issue** — not in each hub implementation issue (see step 9).
